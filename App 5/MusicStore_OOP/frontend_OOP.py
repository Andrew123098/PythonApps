...
class Commands:
    ...
    def search_command(self):
        ...
        for row in database.search(title_text.get(), artist_text.get(), album_text.get(), year_text.get()):
            list1.insert(END, row)  # Insert each tuple in a row in the list box.

    ## Add song to music database
    def add_command(self):
        database.insert(title_text.get(), artist_text.get(), album_text.get(), year_text.get())    # Insert values from entry boxes into music library
        list1.delete(0,END)                                                                        # Clear the list box.
    ...
    def update_command(self): ## >> Call backend update function with selected id and new values from the entry boxes.
        database.update(selected_tuple[0],title_text.get(), artist_text.get(), album_text.get(), year_text.get()) 
...
commands=Commands() # Create commands object to store Commands class functions.
...
list1.bind('<<ListboxSelect>>', Commands.get_selected_row)
...
b1=Button(window,text="View All",width=12, command=commands.view_command)
b1.grid(row=2,column=3)

b2=Button(window,text="Search Entry",width=12, command=commands.search_command)
b2.grid(row=3,column=3)

b3=Button(window,text="Add Entry",width=12, command=commands.add_command)
b3.grid(row=4,column=3)

b4=Button(window,text="Update",width=12, command=commands.update_command)
b4.grid(row=5,column=3)

b5=Button(window,text="Delete",width=12, command=commands.delete_command)
b5.grid(row=6,column=3)

b6=Button(window,text="Clear",width=12, command=commands.clear_command)
